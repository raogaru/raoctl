...
select 
	...
	to_char(time,'DD.MM.YYYY HH24:MI:SS') time, 
	event_name,  sum(delta_total_waits) total_waits, 
	round(sum(delta_time_waited/1000000),3) total_time_s, 
	round(sum(delta_time_waited)/decode(sum(delta_total_waits),0,null,sum(delta_total_waits))/1000,3) avg_time_ms 
from 
	(select 
	hse.instance_number, 
	hse.snap_id, 
...
group by 
	instance_number, time, event_name
order by 
	1, 3, a.time
;
